...
That paragraph is the one field list on this page that nothing checks. `guardrails/0` is configuration the control plane consumes, not a contract it serves, so it has no place in `openapi.json` and no contract test compares it to [`config/control-plane/guardrails.json`](../config/control-plane/guardrails.json). The two can drift apart silently today. What fixes it is policy administration — threat-modelled in roadmap task 1.3 and surfaced in Sprint 2, whose definition of done requires audit events for policy changes to be queryable. Once policy has a served surface, it belongs in `openapi.json` and under the same test as everything else.
