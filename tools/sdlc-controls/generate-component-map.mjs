...
const workspaceComponents = [
  {
    ...
    match: ['tools/sdlc-controls/**', '.github/workflows/sdlc-controls.yml'],
    criticality: 'critical',
    shared: false,
  },
  {
    id: 'control-plane-policy',
    match: ['config/control-plane/**', 'docs/adr/**'],
    criticality: 'critical',
    shared: false,
  },
  ...
  {
    id: 'workspace-config',
    match: ['package.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml', 'nx.json'],
    criticality: 'high',
    shared: false,
  },
...
export function buildComponentMap(graph) {
  ...
  const projects = Object.entries(graph.nodes ?? {}).map(([id, node]) => {
    ...
    const criticality =
      (data.tags ?? []).find((tag) => tag.startsWith('criticality:'))?.slice(12) ?? 'high';
    ...
    return {
      id,
      match: [`${data.root.replace(/\/$/, '')}/**`],
      criticality,
      shared: (counts.get(id) ?? 0) >= 3,
    };
  });
  return {
    version: 1,
    defaults: { unmatched_path_tier: 'high', breadth_threshold: 4 },
    components: [...workspaceComponents, ...projects].sort((a, b) => a.id.localeCompare(b.id)),
  };
}
  ...
  return (
    [
      ...
      ...map.components.flatMap((component) => [
        ...
        `    match: [${component.match.map(quote).join(', ')}]`,
